...
def fun1(ht_url,q2):
    ...
    safe_name = ''.join(c for c in name[-1] if c.isalnum() or c in ('-', '_'))

    path =f"/Experimental Data/实验室项目/github数据/{name[-2]}&{safe_name}&blob&master"
    ...
    while True:
        if os.path.exists(path) and os.listdir(path):
            try:
                ...
                # 如果删除失败，可以选择跳过当前仓库克隆，或者进行其他处理，这里简单跳过
                break
        try:
            ...
            print(f"克隆 {html_url} 失败，尝试第 {a + 1} 次，错误信息: {e}")

            a += 1

            if a == 6:

                print(f"经过6次尝试，克隆 {html_url} 仍失败，放弃操作。")
...
def list_files(folder_path):
    ...
    dst_path = "/Experimental Data/实验室项目/6.8-9.12/"
    """
        这是看我们具体需要那些代码文件
    """
...
if __name__ == '__main__':
    ...
    for file in file_type:
        if not os.path.exists("/Experimental Data/实验室项目/6.8-9.12/{}".format(file)):
            os.makedirs("/Experimental Data/实验室项目/6.8-9.12/{}".format(file))
